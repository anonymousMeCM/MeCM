...
from MetaLearner import AE_TaskEncoder,HGAN,KV_TaskMemory,Modulation_Network
...
class MeCM(torch.nn.Module):
    def __init__(self, config, model_name):
        ...
        print("Parameters of Base model {}!".format(config["base_model"]))
        ...
        # Modulation network
        self.modulation_parameter_list = ["recomm_fc_w2", "recomm_fc_w3"]  #
        self.modulation_net = Modulation_Network(config, self.recomm_module.update_parameters(),
                                                 self.modulation_parameter_list)
    ...
    def global_update(self,task_ids, support_xs_u, support_xs_v, support_ys, query_xs_u,query_xs_v,query_ys, device='cpu'):
        ...
        # Total_loss = loss + self.lambda_recons_loss * recons_loss
        Total_loss = loss
        Total_loss.backward()
    ...
    def local_update(self,support_set_x_u,support_set_x_v, support_set_y, query_set_x_u,query_set_x_v, query_set_y, meta_path_neighbors, mode="train"):
        ...
        #  Global Memory —— Task Cluster level
        Final_task_repre = task_relation_repre
        ...
        # Modulation Network —— Task level
        # modulation_gatings = self.modulation_net(torch.cat([task_relation_repre,task_repre],0))
        modulation_gatings = self.modulation_net(task_repre)

        customized_initial_weights = {}
        for para_i, para_name in enumerate(rec_initial_weights.keys()):
            if para_name in modulation_gatings.keys():
                customized_initial_weights[para_name] = (rec_initial_weights[para_name] - self.customization_ratio *
                                                         initial_fast_weights[para_i]) * (modulation_gatings[para_name].reshape(rec_initial_weights[para_name].shape))
            else:
        ...
        # print("real:",query_y_real)
        # print("pred:",query_y_pred)
